...
public class ActiveCircles extends JFrame implements ActionListener {
	private static final long serialVersionUID = 8252212031536838534L;
	...
	/**
	 * @param args Contains nothing for this program.
	 */
	 ...
	 * @param height          The height of the program's inside portion of the
	 *                        frame
	 */
	public ActiveCircles(int numberOfCircles, int width, int height) {
		...
		for (int i = 0; i < circles.length; i++) {
			circles[i] = new MovingCircle(random.nextFloat() * (getWidth() - circleWidth),
					random.nextFloat() * (getHeight() - circleHeight), circleWidth, circleHeight, random.nextBoolean(),
					random.nextBoolean(), random.nextFloat() * maxSpeed);
		}

		// Setting up the swing components
		JPanel programTitlePanel = new JPanel(new FlowLayout());
		programTitlePanel.add(new JLabel("Actively rendering graphics and Swing components together!"));
		...
		drawing = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration()
	...
	public void draw(Graphics2D g) {
		...
		// render Swing.
		g.drawImage(drawing, insets.left, insets.top, this.getWidth() - (insets.left + insets.right),
				this.getHeight() - (insets.top + insets.bottom), null);
	...
	class MovingCircle {
		...
		public MovingCircle(float x, float y, int circleWidth, int circleHeight, boolean down, boolean right,
				float speed) {
			this.x = x;
			this.y = y;
		 ...
		 * collisions.
		 * 
		 * @param elapsedTime The time that has elapsed since the last time the circle
		 *                    was updated.
		 */
		public void update(long elapsedTime) {
	...
	/**
	 * NoRepaintManager is a RepaintManager that removes the functionality of the
